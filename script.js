...
// 
const ideaBank = [
  "Talk to 5 people who face the same challenge",
  "Map the problem before jumping to solutions",
  "Break it into the smallest solvable piece",
  "Reframe it: what is the opposite problem?",
  "Set a 2-hour timer for brainstorming",
  "Write 10 bad ideas first to unlock the good ones",
  "Research what others have already tried",
  "Find a mentor who solved something similar",
  "Take a 30-minute walk and come back fresh",
  "Write the problem as a question, not a statement",
  "Create a mind map of all related ideas",
  "Start with the end goal and work backwards",
  "Share your challenge with someone outside your field",
  "Look for inspiration in a completely different industry",
  "Combine two existing ideas into something new",
  "Ask: what would I do if failure was impossible?",
  "Reduce the scope by 50% and focus on the core",
  "Write a one-sentence version of your challenge",
  "Set a 5-minute timer and write without stopping",
  "List 3 constraints, then find ways around each one"
];
...
function renderIdeas(ideas) {
  ...
  const badge = document.getElementById('ideas-count');
  if (!list) return;
  ...
  if (badge) badge.textContent = ideas.length;
}
...
// =============================================
// COUNTER ANIMATION (une seule version propre)
// =============================================
  ...
  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting && !started) {
        ...
        setTimeout(() => animateCounter(counters[1], 1600), 250);
        setTimeout(() => animateCounter(counters[2], 1500), 500);
        observer.disconnect();
      }
    });
  }, { threshold: 0.4 });
...
// Hover sur les boxes compteurs
const boxes = document.querySelectorAll('.box4, .box5, .box6');
boxes.forEach(box => {
  box.addEventListener('mouseenter', () => {
    box.style.transform = "translateY(-8px) scale(1.02)";
    box.style.boxShadow = "0 10px 30px rgba(47,62,70,.18)";
    const number = box.querySelector('.counter');
    number.animate(
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function openModal() {
    ...
    style.textContent = `
      ...
      .tm-box {
        background: #f5f2ea; border-radius: 16px; padding: 1.5rem;
      ...
      .tm-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.2rem; }
      .tm-header h3 { font-size: 16px; font-weight: 700; color: #1a2e1a; font-family: Georgia, serif; margin: 0; }
      .tm-close { background: none; border: none; font-size: 16px; color: #888; cursor: pointer; }
      .tm-close:hover { color: #1a2e1a; }
      .tm-label { display: block; font-size: 11px; font-weight: 600; color: #777; font-family: sans-serif; text-transform: uppercase; letter-spacing: .5px; margin: 14px 0 6px; }
      .tm-input { width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid rgba(0,0,0,0.12); background: #fff; font-size: 13px; font-family: sans-serif; color: #1a2e1a; outline: none; box-sizing: border-box; transition: border-color .15s; }
      .tm-input:focus { border-color: #3a7a3a; }
      .tm-input.error { border-color: #e24b4a; }
      .tm-group { display: flex; gap: 7px; flex-wrap: wrap; }
      .tm-chip { padding: 5px 13px; border-radius: 999px; border: 1px solid #c0bbb0; background: #fff; font-size: 12px; font-family: sans-serif; color: #555; cursor: pointer; transition: all .15s; }
      .tm-chip:hover { border-color: #3a7a3a; color: #3a7a3a; }
      .tm-chip.active { background: #2d6a2d; color: #fff; border-color: #2d6a2d; }
      .tm-footer { display: flex; justify-content: flex-end; gap: 10px; margin-top: 1.4rem; }
      .tm-btn-cancel { padding: 8px 18px; border-radius: 9px; border: 1px solid #c0bbb0; background: transparent; font-size: 13px; color: #666; cursor: pointer; font-family: sans-serif; }
      .tm-btn-cancel:hover { border-color: #888; }
      .tm-btn-confirm { padding: 8px 18px; border-radius: 9px; background: #2d6a2d; border: none; color: #fff; font-size: 13px; font-weight: 600; cursor: pointer; font-family: sans-serif; transition: background .15s; }
    ...
    dots.addEventListener('click', e => {
      ...
      menu.innerHTML = `
        <button class="dots-delete-btn" style="
          ...
          font-size: 13px; font-family: sans-serif; color: #e24b4a;
          cursor: pointer; text-align: left;">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#e24b4a" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">
  ...
  function enableToggle(item) {
    ...
    box.addEventListener('click', () => {
      ...
      if (done) {
        ...
        box.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>`;
      } else {
  ...
  function syncHighlight() {
    ...
    if (!hl) return;
    const first = [...document.querySelectorAll('.task-item')].find(t => !t.classList.contains('completed'));
    hl.textContent = first ? first.querySelector('.task-name').textContent : 'All tasks done! 🎉';
  }
